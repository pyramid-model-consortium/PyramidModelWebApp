...
{
    public partial class RptProgramActionPlanFCC : Pyramid.Reports.PreBuiltReports.MasterReports.RptFormReportMaster
    {
        public RptProgramActionPlanFCC()
        {
            InitializeComponent();
        }
        ...
        /// <param name="e"></param>
        private void RptProgramActionPlanFCC_BeforePrint(object sender, System.ComponentModel.CancelEventArgs e)
        {
            //To hold the necessary objects
            Models.ProgramActionPlanFCC currentActionPlan;
            List<Models.PLTMember> activePLTMembers = new List<PLTMember>();
            List<Models.ProgramActionPlanFCCMeeting> currentMeetings = new List<ProgramActionPlanFCCMeeting>();
            List<Models.ProgramLCMeetingSchedule> currentLCMeetingSchedules = new List<ProgramLCMeetingSchedule>();
            List<Models.ProgramActionPlanFCCGroundRule> currentGroundRules = new List<ProgramActionPlanFCCGroundRule>();
            ...
            Models.BenchmarkOfQualityFCC mostRecentBOQ = new BenchmarkOfQualityFCC();
            List<spGetBOQFCCIndicatorValues_Result> mostRecentBOQIndicators = new List<spGetBOQFCCIndicatorValues_Result>();
            List<spGetBOQFCCIndicatorValues_Result> mostRecentBOQIndicatorsThatNeedImprovement = new List<spGetBOQFCCIndicatorValues_Result>();
            List<Models.ProgramActionPlanFCCActionStep> currentActionSteps = new List<ProgramActionPlanFCCActionStep>();
            ...
            using (PyramidContext context = new PyramidContext())
            {
                ...
                currentActionPlan = context.ProgramActionPlanFCC
                                            .Include(ap => ap.Program)
                                            .Include(ap => ap.Program.Cohort)
                                            .AsNoTracking()
                                            .Where(ap => ap.ProgramActionPlanFCCPK == formPK)
                                            .FirstOrDefault();
                ...
                if (currentActionPlan != null)
                {
                    ...
                    currentMeetings = context.ProgramActionPlanFCCMeeting
                                                .AsNoTracking()
                                                .Where(m => m.ProgramActionPlanFCCFK == formPK)
                                                .ToList();
                    ...
                    currentGroundRules = context.ProgramActionPlanFCCGroundRule
                                                .AsNoTracking()
                                                .Where(gr => gr.ProgramActionPlanFCCFK == formPK)
                                                .ToList();
                    ...
                    mostRecentBOQ = context.BenchmarkOfQualityFCC.AsNoTracking()
                    ...
                    if (mostRecentBOQ != null && mostRecentBOQ.BenchmarkOfQualityFCCPK > 0)
                    {
                        ...
                        mostRecentBOQIndicators = context.spGetBOQFCCIndicatorValues(mostRecentBOQ.BenchmarkOfQualityFCCPK).ToList();

                        //Get the indicators that need improvement
                        mostRecentBOQIndicatorsThatNeedImprovement = mostRecentBOQIndicators.Where(iv => iv.IndicatorValue != (int)CodeBOQIndicatorValue.BOQFCCV2IndicatorValues.IN_PLACE).OrderBy(iv => iv.IndicatorNumber).ToList();
                    }

                    currentActionSteps = context.ProgramActionPlanFCCActionStep
                                                .Include(s => s.CodeBOQIndicator)
                                                .Include(s => s.CodeBOQIndicator.CodeBOQCriticalElement)
                                                .Include(s => s.ProgramActionPlanFCCActionStepStatus)
                                                .Include(s => s.ProgramActionPlanFCCActionStepStatus.Select(ss => ss.CodeActionPlanActionStepStatus))
                                                .AsNoTracking()
                                                .Where(s => s.ProgramActionPlanFCCFK == formPK)
                                                .ToList();
                }
            }
            ...
            //Set the detail band label expressions
            lblGRGroundRule.ExpressionBindings.Add(new ExpressionBinding("BeforePrint", "Text", "GroundRuleDescription"));
            ...
            lblBICriticalElement.ExpressionBindings.Add(new ExpressionBinding("BeforePrint", "StyleName", string.Format("Iif([IndicatorValue] == {0}, '{1}', [IndicatorValue] == {2}, '{3}', '{4}')", 
                (int)CodeBOQIndicatorValue.BOQFCCV2IndicatorValues.PARTIALLY_IN_PLACE, WarningStyle.Name,
                (int)CodeBOQIndicatorValue.BOQFCCV2IndicatorValues.NOT_IN_PLACE, DangerStyle.Name, 
                StandardStyle.Name)));
            ...
            ActionStepStatusDetailReport.DataMember = "ProgramActionPlanFCCActionStepStatus";
