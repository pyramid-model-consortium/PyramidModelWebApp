...
namespace Pyramid.Pages
{
    public partial class TPITOSDashboard : System.Web.UI.Page
    {
        ...
        public CodeProgramRolePermission TPITOSPermissions { get; set; }
        public CodeProgramRolePermission FormSchedulePermissions { get; set; }

        private ProgramAndRoleFromSession currentProgramRole;
        private int TPITOSCodeFormPK;
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            TPITOSPermissions = FormPermissions.Where(fp => fp.CodeForm.FormAbbreviation == "TPITOS").FirstOrDefault();
            FormSchedulePermissions = TPITOSPermissions;  //Use the TPITOS permissions for the schedules for now

            //Get the CodeFormPK for the TPITOS CodeForm row
            TPITOSCodeFormPK = TPITOSPermissions.CodeFormFK;
            ...
            SetPermissions(TPITOSPermissions, divTPITOS, bsGRTPITOS, hfTPITOSViewOnly, showStateColumns);
        ...
        {
            if (TPITOSPermissions.AllowedToDelete)
            {
                ...
                if (removeTPITOSPK.HasValue)
                {
                    try
                    {
                        using (PyramidContext context = new PyramidContext())
                        {
                            ...
                            var TPITOSToRemove = context.TPITOS.Where(t => t.TPITOSPK == removeTPITOSPK).FirstOrDefault();
                            ...
                            List<TPITOSParticipantChanged> participantChangeRows;
                            ...
                            if (participantsToRemove.Count > 0)
                            {
                                ...
                                participantChangeRows = context.TPITOSParticipantChanged.Where(tpc => tpc.TPITOSFK == TPITOSToRemove.TPITOSPK)
                            ...
                            if (redFlagsToRemove.Count > 0)
                            {
                                ...
                                redFlagsChangeRows = context.TPITOSRedFlagsChanged.Where(trfc => trfc.TPITOSFK == TPITOSToRemove.TPITOSPK)
                            ...
                            //Get the TPITOS delete row and set the deleter
                            context.TPITOSChanged
                                    .OrderByDescending(tc => tc.TPITOSChangedPK)
                                    .Where(tc => tc.TPITOSPK == TPITOSToRemove.TPITOSPK)
                    ...
                    catch (DbUpdateException dbUpdateEx)
                    {
                        ...
                        if (dbUpdateEx.InnerException?.InnerException is SqlException)
                        {
                            ...
                            if (sqlEx.Number == 547)
                            {
                                ...
                                msgSys.ShowMessageToUser("danger", "Error", string.Format("Could not delete the TPITOS observation, {0}", messageForUser), 120000);
                            }
                            else
                            {
                                msgSys.ShowMessageToUser("danger", "Error", "An error occurred while deleting the TPITOS observation!", 120000);
                            }
                        }
                        else
                        {
        ...
        /// and it highlights incomplete TPITOS observations
        /// </summary>
        ...
        protected void bsGRTPITOS_HtmlRowPrepared(object sender, DevExpress.Web.ASPxGridViewTableRowEventArgs e)
        {
            ...
            {
                //Determine if this row represents a complete TPITOS
                bool isTPITOSComplete = Convert.ToBoolean(e.GetValue("IsComplete"));

                if(!isTPITOSComplete)
                {
                    //The TPITOS is not complete, add the CSS class (deactivated on 05/27/2020 as per Vicki's request)
                    //e.Row.CssClass = "incomplete-tpitos";
                }
            }
        }
        ...
        protected void efFormScheduleDataSource_Selecting(object sender, DevExpress.Data.Linq.LinqServerModeDataSourceSelectEventArgs e)
        {
            ...
            e.QueryableSource = context.FormSchedule
                                        ...
                                        .AsNoTracking()
                                        .Where(fs => fs.CodeFormFK == TPITOSCodeFormPK &&  //Only get the TPITOS rows
                    ...
                    using (PyramidContext context = new PyramidContext())
                    {
                        ...
                        List<FormSchedule> duplicateSchedules = context.FormSchedule.AsNoTracking()
                                        .Where(s => s.ProgramFK == selectedProgramFK.Value &&
                                                ...
                                                s.CodeFormFK == TPITOSCodeFormPK).ToList();
            ...
            if (FormSchedulePermissions.AllowedToEdit)
            {
                ...
                using (PyramidContext context = new PyramidContext())
                {
                    ...
                    if (formSchedulePK == 0)
                    {
                        ...
                        currentSchedule.CodeFormFK = TPITOSCodeFormPK;
