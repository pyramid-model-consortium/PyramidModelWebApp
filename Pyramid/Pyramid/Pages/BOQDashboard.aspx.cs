...
namespace Pyramid.Pages
{
    public partial class BOQDashboard : System.Web.UI.Page
    {
        ...
        public CodeProgramRolePermission BOQPermissions { get; set; }
        public CodeProgramRolePermission FormSchedulePermissions { get; set; }

        private ProgramAndRoleFromSession currentProgramRole;
        private int BOQCodeFormPK;
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            BOQPermissions = FormPermissions.Where(fp => fp.CodeForm.FormAbbreviation == "BOQ").FirstOrDefault();
            FormSchedulePermissions = BOQPermissions;  //Use the BOQ permissions for the schedules for now

            //Get the CodeFormPK for the BOQ CodeForm row
            BOQCodeFormPK = BOQPermissions.CodeFormFK;
            ...
            SetPermissions(BOQPermissions, divBOQs, bsGRBOQs, hfBOQViewOnly, showStateColumns);
        ...
        {
            //To hold all the non-fcc programs
            List<Program> allNonFCCPrograms = new List<Program>();

            //Get all the non-fcc programs
            using (PyramidContext context = new PyramidContext())
            {
                allNonFCCPrograms = context.Program
            ...
            //Bind the non-fcc program dropdowns
            ddFSProgram.DataSource = allNonFCCPrograms;
        ...
        protected void efBOQDataSource_Selecting(object sender, DevExpress.Data.Linq.LinqServerModeDataSourceSelectEventArgs e)
        {
            ...
            e.QueryableSource = context.BenchmarkOfQuality.AsNoTracking()
                                        .Include(boq => boq.Program)
                                        .Include(boq => boq.Program.State)
        ...
        {
            if (BOQPermissions.AllowedToDelete)
            {
                ...
                if (removeBOQPK.HasValue)
                {
                    try
                    {
                        using (PyramidContext context = new PyramidContext())
                        {
                            ...
                            Models.BenchmarkOfQuality BOQToRemove = context.BenchmarkOfQuality.Where(boq => boq.BenchmarkOfQualityPK == removeBOQPK).FirstOrDefault();
                            ...
                            context.BenchmarkOfQualityChanged
                                    .OrderByDescending(boqc => boqc.BenchmarkOfQualityChangedPK)
                                    .Where(boqc => boqc.BenchmarkOfQualityPK == BOQToRemove.BenchmarkOfQualityPK)
                    ...
                    catch (DbUpdateException dbUpdateEx)
                    {
                        ...
                        if (dbUpdateEx.InnerException?.InnerException is SqlException)
                        {
                            ...
                            if (sqlEx.Number == 547)
                            {
                                ...
                                msgSys.ShowMessageToUser("danger", "Error", string.Format("Could not delete the Benchmark of Quality 2.0 form, {0}", messageForUser), 120000);
                            }
                            else
                            {
                                msgSys.ShowMessageToUser("danger", "Error", "An error occurred while deleting the Benchmark of Quality 2.0 form!", 120000);
                            }
                        }
                        else
                        {
        ...
        protected void efFormScheduleDataSource_Selecting(object sender, DevExpress.Data.Linq.LinqServerModeDataSourceSelectEventArgs e)
        {
            ...
            e.QueryableSource = context.FormSchedule
                                        ...
                                        .AsNoTracking()
                                        .Where(fs => fs.CodeFormFK == BOQCodeFormPK &&  //Only get the BOQ 2.0 rows
                    ...
                    using (PyramidContext context = new PyramidContext())
                    {
                        ...
                        List<FormSchedule> duplicateSchedules = context.FormSchedule.AsNoTracking()
                                        .Where(s => s.ProgramFK == selectedProgramFK.Value &&
                                                ...
                                                s.CodeFormFK == BOQCodeFormPK).ToList();
            ...
            if (FormSchedulePermissions.AllowedToEdit)
            {
                ...
                using (PyramidContext context = new PyramidContext())
                {
                    ...
                    if (formSchedulePK == 0)
                    {
                        ...
                        currentSchedule.CodeFormFK = BOQCodeFormPK;
