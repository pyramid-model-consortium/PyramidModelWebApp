...
{
    public partial class ProgramActionPlan : System.Web.UI.Page, IForm
    {
        ...
        private Models.ProgramActionPlan currentActionPlan;
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrWhiteSpace(Request.QueryString["ProgramActionPlanPK"]))
            {
                //Parse the pk
                int.TryParse(Request.QueryString["ProgramActionPlanPK"], out currentActionPlanPK);
            }

            //If the current PK is 0, try to get the value from the hidden field
            if (currentActionPlanPK == 0 && !string.IsNullOrWhiteSpace(hfProgramActionPlanPK.Value))
            {
                int.TryParse(hfProgramActionPlanPK.Value, out currentActionPlanPK);
            }
            ...
            using (PyramidContext context = new PyramidContext())
            {
                ...
                currentActionPlan = context.ProgramActionPlan.AsNoTracking()
                                        .Include(d => d.Program)
                                        .Include(d => d.Program.Cohort)
                                        .Where(d => d.ProgramActionPlanPK == currentActionPlanPK).FirstOrDefault();
                ...
                if (currentActionPlan == null)
                {
                    ...
                    currentActionPlan = new Models.ProgramActionPlan();
                }
                else
                {
                    //Show a message to the user if this is not the most recent action plan for the program
                    int numMoreRecentPlans = context.ProgramActionPlan.AsNoTracking()
                                                    .Where(ap => ap.ProgramActionPlanPK != currentActionPlan.ProgramActionPlanPK &&
                    ...
                    if (numMoreRecentPlans > 0)
                    {
                        ...
                        lblActionPlanAlert.Text = "This is not the most recent action plan for this program.  Any new or changed information should be added to the most recent action plan.";
                    }
                }
            }
        ...
        private void BindDropDowns()
        {
            ...
            using (PyramidContext context = new PyramidContext())
            {
                allPrograms = context.Program.AsNoTracking()
                                             ...
                                             .ToList();
                allIndicators = context.CodeBOQIndicator.Include(i => i.CodeBOQCriticalElement).AsNoTracking().Where(i => i.CodeBOQCriticalElement.BOQTypeCodeFK == (int)CodeBOQType.BOQTypes.BOQ).OrderBy(i => i.OrderBy).ToList();
        ...
        protected void submitActionPlan_Click(object sender, EventArgs e)
        {
            ...
            if (formSaved)
            {
                ...
                else
                {
                    ...
                    Response.Redirect(string.Format("/Pages/ProgramActionPlan.aspx?ProgramActionPlanPK={0}&Action=Edit",
                                                        currentActionPlanPK));
                }
            }
        }
        ...
        private void PrintForm()
        {
            ...
            if (ASPxEdit.AreEditorsValid(this.Page, submitActionPlan.ValidationGroup))
            {
                ...
                if (isEdit)
                {
                    ...
                    //Get the report
                    Reports.PreBuiltReports.FormReports.RptProgramActionPlan report = new Reports.PreBuiltReports.FormReports.RptProgramActionPlan();

                    //Display the report
                    masterPage.DisplayReport(currentProgramRole, report, "Action Plan", currentActionPlanPK);
                }
                else
                {
                    //Get the action
                    ...
                    Response.Redirect(string.Format("/Pages/ProgramActionPlan.aspx?ProgramActionPlanPK={0}&Action={1}&Print=True",
            ...
            if ((isEdit && FormPermissions.AllowedToEdit) || (isEdit == false && FormPermissions.AllowedToAdd))
            {
                ...
                if (isEdit)
                {
                    using (PyramidContext context = new PyramidContext())
                    {
                        ...
                        Models.ProgramActionPlan existingActionPlan = context.ProgramActionPlan.Find(currentActionPlan.ProgramActionPlanPK);
                        ...
                        hfProgramActionPlanPK.Value = currentActionPlan.ProgramActionPlanPK.ToString();
                        currentActionPlanPK = currentActionPlan.ProgramActionPlanPK;
                ...
                else
                {
                    using (PyramidContext context = new PyramidContext())
                    {
                        ...
                        context.ProgramActionPlan.Add(currentActionPlan);
        ...
        protected void ddProgram_Validation(object sender, ValidationEventArgs e)
        {
            ...
            else if (currentActionPlan.IsPrefilled == false || chkReviewedBasicInfo.Checked)
            {
                ...
                if (deActionPlanStartDate.Date != DateTime.MinValue && deActionPlanEndDate.Date != DateTime.MinValue)
                {
                    using (PyramidContext context = new PyramidContext())
                    {
                        //Check to see if there are any schedules for this program in that timeframe
                        List<Models.ProgramActionPlan> duplicateDebriefs = context.ProgramActionPlan.AsNoTracking()
                                        .Where(d => d.ProgramFK == selectedProgramFK.Value &&
                                                d.ProgramActionPlanPK != currentActionPlanPK &&
                ...
                isLeadershipCoachInvolved == true)
            {
                ...
                if (string.IsNullOrWhiteSpace(leadershipCoachUsername))
                {
                    ...
                    e.ErrorText = "Primary Leadership Coach is required!";
                            ...
                            (currentActionPlan.IsPrefilled == false || chkReviewedBasicInfo.Checked))
                {
                    //Only do this validation if a program was selected, a coach was selected,
                    //and the form was either not prefilled or the section was reviewed
                    using (PyramidContext context = new PyramidContext())
                    {
        ...
        protected void deActionPlanStartDate_Validation(object sender, ValidationEventArgs e)
        {
            ...
            //Check to see if the debrief start date was selected
        ...
        protected void deActionPlanEndDate_Validation(object sender, ValidationEventArgs e)
        {
            ...
            //Check to see if the debrief end date was selected
            ...
            else if (actionPlanStartDate != DateTime.MinValue)
            {
                ...
                else if (isEdit)
                {
                    using (PyramidContext context = new PyramidContext())
                    {
                        ...
                        List<Models.ProgramActionPlanMeeting> invalidMeetingDates = context.ProgramActionPlanMeeting
                                                    .AsNoTracking()
                                                    .Where(m => m.ProgramActionPlanFK == currentActionPlan.ProgramActionPlanPK &&
                        ...
                        if (currentActionPlan.IsPrefilled == false || chkReviewedActionSteps.Checked)
                        {
                            ...
                            List<Models.ProgramActionPlanActionStep> invalidActionSteps = context.ProgramActionPlanActionStep
                                                        .Include(s => s.CodeBOQIndicator)
                                                        .Include(s => s.CodeBOQIndicator.CodeBOQCriticalElement)
                                                        .AsNoTracking()
                                                        .Where(s => s.ProgramActionPlanFK == currentActionPlan.ProgramActionPlanPK &&
        ...
        private void BindMeetings()
        {
            ...
            {
                //Bind the repeater
                List<ProgramActionPlanMeeting> Meetings = context.ProgramActionPlanMeeting.AsNoTracking()
                                            .Where(t => t.ProgramActionPlanFK == currentActionPlan.ProgramActionPlanPK)
                                            .ToList();
        ...
        protected void lbViewMeeting_Click(object sender, EventArgs e)
        {
            ...
            if (meetingPK.HasValue)
            {
                using (PyramidContext context = new PyramidContext())
                {
                    ...
                    ProgramActionPlanMeeting currentMeeting = context.ProgramActionPlanMeeting.AsNoTracking().Where(m => m.ProgramActionPlanMeetingPK == meetingPK.Value).FirstOrDefault();
        ...
        protected void lbEditMeeting_Click(object sender, EventArgs e)
        {
            ...
            if (meetingPK.HasValue)
            {
                using (PyramidContext context = new PyramidContext())
                {
                    ...
                    ProgramActionPlanMeeting editMeeting = context.ProgramActionPlanMeeting.AsNoTracking().Where(m => m.ProgramActionPlanMeetingPK == meetingPK.Value).FirstOrDefault();
        ...
        protected void submitMeeting_Click(object sender, EventArgs e)
        {
            ...
            if (FormPermissions.AllowedToEdit)
            {
                ...
                {
                    //To hold the object
                    ProgramActionPlanMeeting currentMeeting;

                    //Check to see if this is an add or an edit
                    if (meetingPK == 0)
                    {
                        //Add
                        currentMeeting = new ProgramActionPlanMeeting();
                        ...
                        currentMeeting.ProgramActionPlanFK = currentActionPlanPK;
                        currentMeeting.CreateDate = DateTime.Now;
                        currentMeeting.Creator = User.Identity.Name;

                        //Save to the database
                        context.ProgramActionPlanMeeting.Add(currentMeeting);
                        context.SaveChanges();
                    ...
                    {
                        //Edit
                        currentMeeting = context.ProgramActionPlanMeeting.Find(meetingPK);
                        currentMeeting.MeetingDate = deMeetingDate.Date;
                        currentMeeting.LeadershipCoachAttendance = Convert.ToBoolean(ddMeetingLeadershipCoachAttendance.Value);
                        currentMeeting.MeetingNotes = (string.IsNullOrWhiteSpace(txtMeetingNotes.Text) ? null : txtMeetingNotes.Text);
                ...
                if (rowToRemovePK.HasValue)
                {
                    try
                    {
                        using (PyramidContext context = new PyramidContext())
                        {
                            ...
                            ProgramActionPlanMeeting meetingToRemove = context.ProgramActionPlanMeeting.Where(t => t.ProgramActionPlanMeetingPK == rowToRemovePK).FirstOrDefault();

                            //Remove the meeting
                            context.ProgramActionPlanMeeting.Remove(meetingToRemove);

                            //Save the deletion to the database
                            context.SaveChanges();

                            //Get the delete change row and set the deleter
                            context.ProgramActionPlanMeetingChanged
                                    .OrderByDescending(c => c.ProgramActionPlanMeetingChangedPK)
                                    .Where(c => c.ProgramActionPlanMeetingPK == meetingToRemove.ProgramActionPlanMeetingPK)
        ...
        private void BindGroundRules()
        {
            ...
            {
                //Bind the repeater
                List<ProgramActionPlanGroundRule> GroundRules = context.ProgramActionPlanGroundRule.AsNoTracking()
                                            .Where(t => t.ProgramActionPlanFK == currentActionPlan.ProgramActionPlanPK)
                                            .ToList();
        ...
        protected void lbViewGroundRule_Click(object sender, EventArgs e)
        {
            ...
            if (groundRulePK.HasValue)
            {
                using (PyramidContext context = new PyramidContext())
                {
                    ...
                    ProgramActionPlanGroundRule currentGroundRule = context.ProgramActionPlanGroundRule.AsNoTracking().Where(m => m.ProgramActionPlanGroundRulePK == groundRulePK.Value).FirstOrDefault();
        ...
        protected void lbEditGroundRule_Click(object sender, EventArgs e)
        {
            ...
            if (groundRulePK.HasValue)
            {
                using (PyramidContext context = new PyramidContext())
                {
                    ...
                    ProgramActionPlanGroundRule editGroundRule = context.ProgramActionPlanGroundRule.AsNoTracking().Where(m => m.ProgramActionPlanGroundRulePK == groundRulePK.Value).FirstOrDefault();
        ...
        protected void submitGroundRule_Click(object sender, EventArgs e)
        {
            ...
            if (FormPermissions.AllowedToEdit)
            {
                ...
                {
                    //To hold the object
                    ProgramActionPlanGroundRule currentGroundRule;

                    //Check to see if this is an add or an edit
                    if (groundRulePK == 0)
                    {
                        //Add
                        currentGroundRule = new ProgramActionPlanGroundRule();
                        ...
                        currentGroundRule.ProgramActionPlanFK = currentActionPlanPK;
                        currentGroundRule.CreateDate = DateTime.Now;
                        currentGroundRule.Creator = User.Identity.Name;

                        //Save to the database
                        context.ProgramActionPlanGroundRule.Add(currentGroundRule);
                        context.SaveChanges();
                    ...
                    {
                        //Edit
                        currentGroundRule = context.ProgramActionPlanGroundRule.Find(groundRulePK);
                        currentGroundRule.GroundRuleDescription = txtGroundRuleDescription.Text;
                        currentGroundRule.GroundRuleNumber = Convert.ToInt32(txtGroundRuleNumber.Text);
                ...
                if (rowToRemovePK.HasValue)
                {
                    try
                    {
                        using (PyramidContext context = new PyramidContext())
                        {
                            ...
                            ProgramActionPlanGroundRule assignmentToRemove = context.ProgramActionPlanGroundRule.Where(t => t.ProgramActionPlanGroundRulePK == rowToRemovePK).FirstOrDefault();

                            //Remove the ground rule
                            context.ProgramActionPlanGroundRule.Remove(assignmentToRemove);

                            //Save the deletion to the database
                            context.SaveChanges();

                            //Get the delete change row and set the deleter
                            context.ProgramActionPlanGroundRuleChanged
                                    .OrderByDescending(c => c.ProgramActionPlanGroundRuleChangedPK)
                                    .Where(c => c.ProgramActionPlanGroundRulePK == assignmentToRemove.ProgramActionPlanGroundRulePK)
                                    .FirstOrDefault().Deleter = User.Identity.Name;
        ...
        protected void txtGroundRuleDescription_Validation(object sender, ValidationEventArgs e)
        {
            ...
            if (groundRulePK == 0)
            {
                ...
                using(PyramidContext context = new PyramidContext())
                {
                    int numOtherGroundRules = context.ProgramActionPlanGroundRule.AsNoTracking().Where(r => r.ProgramActionPlanFK == currentActionPlan.ProgramActionPlanPK).Count();
        ...
        private void BindMostRecentBOQInformation(int? currentProgramFK)
        {
                ...
                currentProgramFK.Value > 0)
            {
                using (PyramidContext context = new PyramidContext())
                {
                    ...
                    BenchmarkOfQuality mostRecentBOQ = context.BenchmarkOfQuality.AsNoTracking()
                    ...
                    if(mostRecentBOQ != null && mostRecentBOQ.BenchmarkOfQualityPK > 0)
                    {
                        ...
                        List<spGetBOQIndicatorValues_Result> mostRecentBOQIndicatorValues = context.spGetBOQIndicatorValues(mostRecentBOQ.BenchmarkOfQualityPK).ToList();

                        //Get the indicators that need improvement
                        List<spGetBOQIndicatorValues_Result> indicatorsThatNeedImprovement = mostRecentBOQIndicatorValues.Where(iv => iv.IndicatorValue != (int)CodeBOQIndicatorValue.BOQIndicatorValues.IN_PLACE).OrderBy(iv => iv.IndicatorNumber).ToList();
                    ...
                    {
                        //Bind the repeaters
                        repeatBOQIndicatorsToBeImproved.DataSource = new List<spGetBOQIndicatorValues_Result>();
                        repeatBOQIndicatorsToBeImproved.DataBind();
        ...
        private void BindActionSteps()
        {
            ...
            {
                //Bind the repeater
                List<ProgramActionPlanActionStep> actionSteps = context.ProgramActionPlanActionStep.AsNoTracking()
                                            .Where(t => t.ProgramActionPlanFK == currentActionPlan.ProgramActionPlanPK)
                                            .ToList();
        ...
        protected void lbViewActionStep_Click(object sender, EventArgs e)
        {
            ...
            if (actionStepPK.HasValue)
            {
                using (PyramidContext context = new PyramidContext())
                {
                    ...
                    ProgramActionPlanActionStep currentActionStep = context.ProgramActionPlanActionStep
                                                                                .Include(a => a.CodeBOQIndicator)
                                                                                .AsNoTracking()
                                                                                .Where(a => a.ProgramActionPlanActionStepPK == actionStepPK.Value)
                                                                                .FirstOrDefault();
        ...
        protected void lbEditActionStep_Click(object sender, EventArgs e)
        {
            ...
            if (actionStepPK.HasValue)
            {
                using (PyramidContext context = new PyramidContext())
                {
                    ...
                    ProgramActionPlanActionStep editActionStep = context.ProgramActionPlanActionStep.AsNoTracking().Where(m => m.ProgramActionPlanActionStepPK == actionStepPK.Value).FirstOrDefault();
        ...
        protected void submitActionStep_Click(object sender, EventArgs e)
        {
            ...
            if (FormPermissions.AllowedToEdit)
            {
                ...
                {
                    //To hold the object
                    ProgramActionPlanActionStep currentActionStep;

                    //Check to see if this is an add or an edit
                    if (actionStepPK == 0)
                    {
                        //Add
                        currentActionStep = new ProgramActionPlanActionStep();
                        ...
                        currentActionStep.ProgramActionPlanFK = currentActionPlanPK;
                        currentActionStep.CreateDate = DateTime.Now;
                        currentActionStep.Creator = User.Identity.Name;

                        //Save to the database
                        context.ProgramActionPlanActionStep.Add(currentActionStep);
                        context.SaveChanges();

                        //Add the initial status record
                        ProgramActionPlanActionStepStatus currentStatusRecord = new ProgramActionPlanActionStepStatus();
                        currentStatusRecord.ProgramActionPlanActionStepFK = currentActionStep.ProgramActionPlanActionStepPK;
                        ...
                        //Save to the database
                        context.ProgramActionPlanActionStepStatus.Add(currentStatusRecord);
                        context.SaveChanges();
                    ...
                    {
                        //Edit
                        currentActionStep = context.ProgramActionPlanActionStep.Find(actionStepPK);
                        currentActionStep.BOQIndicatorCodeFK = Convert.ToInt32(ddActionStepIndicator.Value);
                        currentActionStep.ProblemIssueTask = (string.IsNullOrWhiteSpace(txtActionStepProblemIssueTask.Text) ? null : txtActionStepProblemIssueTask.Text);
                        currentActionStep.ActionStepActivity = (string.IsNullOrWhiteSpace(txtActionStepActivity.Text) ? null : txtActionStepActivity.Text);
                        currentActionStep.PersonsResponsible = (string.IsNullOrWhiteSpace(txtActionStepPersonsResponsible.Text) ? null : txtActionStepPersonsResponsible.Text);
                ...
                if (rowToRemovePK.HasValue)
                {
                    try
                    {
                        using (PyramidContext context = new PyramidContext())
                        {
                            ...
                            ProgramActionPlanActionStep actionStepToRemove = context.ProgramActionPlanActionStep.Where(t => t.ProgramActionPlanActionStepPK == rowToRemovePK).FirstOrDefault();
                            ...
                            var actionStepStatusesToRemove = context.ProgramActionPlanActionStepStatus
                                .Where(ass => ass.ProgramActionPlanActionStepFK == actionStepToRemove.ProgramActionPlanActionStepPK).ToList();
                            context.ProgramActionPlanActionStepStatus.RemoveRange(actionStepStatusesToRemove);

                            //Remove the action step
                            context.ProgramActionPlanActionStep.Remove(actionStepToRemove);
                            ...
                            //Get the delete change row and set the deleter
                            context.ProgramActionPlanActionStepChanged
                                    .OrderByDescending(c => c.ProgramActionPlanActionStepChangedPK)
                                    .Where(c => c.ProgramActionPlanActionStepPK == actionStepToRemove.ProgramActionPlanActionStepPK)
                                    .FirstOrDefault().Deleter = User.Identity.Name;
        ...
        private void BindActionStepStatuses()
        {
            ...
            {
                //Bind the repeater
                List<ProgramActionPlanActionStepStatus> actionStepStatuses = context.ProgramActionPlanActionStepStatus
                                            .Include(s => s.ProgramActionPlanActionStep)
                                            .AsNoTracking()
                                            .Where(s => s.ProgramActionPlanActionStepFK == actionStepFK)
                                            .ToList();
        ...
        protected void lbViewActionStepStatus_Click(object sender, EventArgs e)
        {
            ...
            if (actionStepStatusPK.HasValue)
            {
                using (PyramidContext context = new PyramidContext())
                {
                    ...
                    ProgramActionPlanActionStepStatus currentActionStepStatus = context.ProgramActionPlanActionStepStatus
                                                                                .AsNoTracking()
                                                                                .Where(a => a.ProgramActionPlanActionStepStatusPK == actionStepStatusPK.Value)
                                                                                .FirstOrDefault();
        ...
        protected void lbEditActionStepStatus_Click(object sender, EventArgs e)
        {
            ...
            if (actionStepStatusPK.HasValue)
            {
                using (PyramidContext context = new PyramidContext())
                {
                    ...
                    ProgramActionPlanActionStepStatus editActionStepStatus = context.ProgramActionPlanActionStepStatus.AsNoTracking().Where(m => m.ProgramActionPlanActionStepStatusPK == actionStepStatusPK.Value).FirstOrDefault();
        ...
        protected void submitActionStepStatus_Click(object sender, EventArgs e)
        {
            ...
            if (FormPermissions.AllowedToEdit)
            {
                ...
                {
                    //To hold the object
                    ProgramActionPlanActionStepStatus currentActionStepStatus;

                    //Check to see if this is an add or an edit
                    if (actionStepStatusPK == 0)
                    {
                        //Add
                        currentActionStepStatus = new ProgramActionPlanActionStepStatus();
                        ...
                        currentActionStepStatus.ProgramActionPlanActionStepFK = actionStepFK;
                        currentActionStepStatus.CreateDate = DateTime.Now;
                        currentActionStepStatus.Creator = User.Identity.Name;

                        //Save to the database
                        context.ProgramActionPlanActionStepStatus.Add(currentActionStepStatus);
                        context.SaveChanges();
                    ...
                    {
                        //Edit
                        currentActionStepStatus = context.ProgramActionPlanActionStepStatus.Find(actionStepStatusPK);
                        currentActionStepStatus.ActionPlanActionStepStatusCodeFK = Convert.ToInt32(ddActionStepStatus.Value);
                        currentActionStepStatus.StatusDate = deActionStepStatusDate.Date;
                        currentActionStepStatus.ProgramActionPlanActionStepFK = actionStepFK;
                ...
                if (rowToRemovePK.HasValue)
                {
                    try
                    {
                        using (PyramidContext context = new PyramidContext())
                        {
                            ...
                            ProgramActionPlanActionStepStatus actionStepStatusToRemove = context.ProgramActionPlanActionStepStatus.Where(t => t.ProgramActionPlanActionStepStatusPK == rowToRemovePK).FirstOrDefault();

                            //Don't allow deletion if there are no other status records for this action step
                            if (context.ProgramActionPlanActionStepStatus.Where(s => s.ProgramActionPlanActionStepStatusPK != actionStepStatusToRemove.ProgramActionPlanActionStepStatusPK &&
                                                                                        s.ProgramActionPlanActionStepFK == actionStepStatusToRemove.ProgramActionPlanActionStepFK)
                                .Count() > 0)
                            {
                                //Remove the action step status
                                context.ProgramActionPlanActionStepStatus.Remove(actionStepStatusToRemove);

                                //Save the deletion to the database
                                context.SaveChanges();

                                //Get the delete change row and set the deleter
                                context.ProgramActionPlanActionStepStatusChanged
                                        .OrderByDescending(c => c.ProgramActionPlanActionStepStatusChangedPK)
                                        .Where(c => c.ProgramActionPlanActionStepStatusPK == actionStepStatusToRemove.ProgramActionPlanActionStepStatusPK)
                                        .FirstOrDefault().Deleter = User.Identity.Name;
